...
| `cylinders[].ppo2_limit` | bar | `<maximumpo2>` | bar | — the one pressure UDDF does not express in Pascal |
...
### Centers — `/uddf/divesite/divebase`, `/uddf/business/shop`, and a trip part's own

§6.18 is one record where UDDF has five shapes, and four of them read into `centers[]`, each
with the role its slot implies:

| UDDF | DiveJSON |
| --- | --- |
| `divesite/divebase` | a center with `roles: ["dive_center"]` |
| `business/shop` | a center with `roles: ["shop"]` — `<business>` is only their container |
| `trippart/accomodation` | a center with `roles: ["accommodation"]` — *Trips* below |
| `trippart/operator` | a center with `roles: ["liveaboard"]` — *Trips* below |
| `name` | `centers[].name` — REQUIRED, so a shape whose `<name>` is empty is no center: it is reported, and every link to it goes with it |
| `address/street`, `city`, `postcode`, `province`, `country` | `centers[].address.street`, `.city`, `.postcode`, `.region`, `.country` |
| `contact/phone`, else `contact/mobilephone` | `centers[].phone` — the first `<phone>`, or the first `<mobilephone>` where there is none |
| `contact/email` | `centers[].email` — the first |
| `contact/homepage` | `centers[].website` — the first |
| `notes/para` | `centers[].notes`, paragraphs joined with blank lines |
| `@id` | `centers[].uuid`, under the one kind `center` whichever slot it came from |
| a dive's `informationbeforedive/link/@ref` naming a base or a shop | `dives[].center_uuid` — *Dives* below |
...
a center, so a reader never fills their `center_uuid`: `<certification><organization>` is
the agency, as text, and not the school.

**`centers[]` lists the bases in file order, then the shops, then the centers the trip parts
add in part order, then those the kit list's purchases add**, and a center's `roles` come in
...
beside a comment that Subsurface does not track the concept, and read as a center it would
...
**A `<purchase>`'s own `<shop>` is read as a center** where no center read before it has its
...
| `trippart/accomodation` | `trips[].parts[].accommodation_uuid`, and a center — *Centers* above |
| `trippart/operator` + `vessel` | `trips[].parts[].accommodation_uuid`, and a center named for the operator |
...
liveaboard, the boat being the bed. Both read into a center the part's `accommodation_uuid`
...
**An inline shape is folded by name into the center it names.** `<accomodation>` and
...
matched on its `<name>`, trimmed and case-insensitively, against every center read before it
...
everything about the boat — is dropped and reported.
...
**The first link is where the dive's own once-per-dive facts land**, all three of them,
because UDDF states each once per **dive** and never once per computer:
...
- **Its `<samples>` become that first recording's profile.** A dive has one `<samples>`
  ...
  after the first is device-only. That profile, with the surface pressure below, is the
  only thing that carries a link past the carve-out without a device, and it reaches exactly
  one link — so **where the dive has a profile**, the first link is a recording whether or
  not its element names a device, exactly as a dive linking no computer at all is one
  recording made of its samples alone, and the carve-out bites only on the *later* links
  whose elements name none. **Where the dive has neither a profile nor a surface pressure** —
  an absent `<samples>`, or samples `converting.md` found wholly unusable — there is nothing
  to protect any link, so every one of them is judged on its device alone, the first
  included, and a dive whose elements all name none has no recordings at all.
- **Its `<surfacepressure>` goes to that first link's recording**, a readout being enough to
  make one (§6.4a) the way a profile is — *Dives* below.
...
| `informationbeforedive/link/@ref` naming a `<divebase>` or a `<shop>` | `center_uuid` |
...
`2002-06-18T` and a bare `2002-06-18` are read as a **date-only start** (§5.2), with the time
of day reported absent — `converting.md`'s rule, and not midnight, which neither string
...
so it goes where `converting.md` sends a dive-level readout: onto the primary recording — the
first link's, *Device* above — reported `resolved` where the dive has more than one, and onto
a recording of its own where the dive yields none. No file in this corpus links two
computers, so the `resolved` case has no pair.
...
decompression model* below) or a center — a `<divebase>` or a `<shop>`, which becomes the
dive's `center_uuid`. UDDF's prose names buddies and sites as this link's targets, while
...
linking two centers keeps the first and reports the rest, §6.2 carrying one. A link that
...
| `link/@ref` → `gasdefinitions/mix/maximumpo2` | `cylinders[].ppo2_limit` |
...
- `<divetime>` is `xs:float` seconds while §6.5's `times` are whole milliseconds, so each is
  multiplied by a thousand and rounded, a fraction the file states is kept, and two waypoints
  that round to one millisecond are `converting.md`'s collision.
  `fixtures/uddf/legacy-writer.uddf` writes a `30.4` beside a `30`: one second, two
  milliseconds, so both readings are kept.
...
- **Two cylinders on one blend link the same `<mix>`**, so a `@ref` resolves to a *list* of
  ...
  would put two readings on one instant, which §6.5 forbids, and would say the diver carried
...
sample span in both real exports on record, 4010 s against 4300 s and 4001 s against 4288 s.
...
profile spanning 4301 s over a logged `duration` of 4001. §6.4 defines the member as the
...
| `<waypoint><otu>` | §6.4 has a `cns` channel and no OTU one, and no file in hand carries this element. Deriving a recording's `otu_start`/`otu_end` from the last sample would present a derivation as recorded data, which §5.7 forbids. |
...
| `<trippart @type>` | `boat`, `hotel`, `individual` or `organized`, and reported. What a part gained was a reference, not a type: the center its `accommodation_uuid` names says whether the diver slept aboard (`liveaboard`) or ashore (`accommodation`), and `individual` and `organized` say how the trip was booked, which nothing stores ([divejson/divejson's CONTRIBUTING.md](https://github.com/divejson/divejson/blob/main/CONTRIBUTING.md#proposing-additions-to-the-data-model)). A reader that wants it has `extensions`. |
| a center's `<aliasname>` and `<rating>`; a base's `<priceperdive>`, `<pricedivepackage>`, `<guide>` and `<link>`; an accommodation's `<category>` | §6.18 carries none of them, and says so under *Deferred*. Each is reported. |
| `<contact><language>`, `<fax>` on a center | no member. Each is reported. |
| `<vessel>`, with its `<shiptype>`, `<marina>` and `<shipdimension>` | a boat, which §6.18 does not model: the operator is the center and the vessel is reported. |
| `<purchase>` | a piece of kit's price, date and shop. Its inline `<shop>` is read as a center (*Centers* above) and the purchase is reported. |
...
- **Subsurface writes one placeholder `<divebase>` into every export** — `allbase`, named
  `Subsurface Divebase`, linked from nothing. *Centers* above is why it does not arrive as a
  center.
