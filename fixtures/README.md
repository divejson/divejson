...
| `header-only.json` | Suunto D5, 2021 | The shape with no gas anywhere: a header and no samples at all, so no cylinders and no profile, and neither reported — the source recorded none rather than a reader failing to carry them. The header does name a device, so the dive carries a **device-only recording** (§6.4a): the corpus's only one, and the case that says a computer worn is a fact about the dive even when it sampled nothing. Constructed rather than reduced; no real export of this shape is in hand. |
