...
| `d5-deco-max-depth.json` | Suunto D5, 2025 | A real decompression dive to 46 m, and the file that settles what a `NoDecTime` of zero is: it writes one at 42.6 to 44.5 m with a time to surface of 256 to 268 s beside it and a ceiling a few samples later, which is a reading and not an absence. Also `Max.Depth` for `depth_alarm` and `Ceiling Broken` for `ceiling_violation`. |
| `ocean-deco-ppo2.json` | Suunto Ocean, 2026 | `PO2 High` for `ppo2_high` and `NoDecoTime` for `ndl_reached`, on a 45 m decompression dive — the deepest file in the corpus. It is also the `gtSurface` pair: this firmware, 2.40.56, spells the surface gradient factor with a `t`, where the 2.51.28 export `suunto-ocean-2026.json` came from spells it `gfSurface`, and both read into the same channel. |
